class Bank
  ...
  def initialize
    @list_of_accounts = []
    @account = Account.new(123456789, 11, 1000)
...
def show_balance_of_the_account(account_number)
  @list_of_accounts.map do |account|
    if account.number == account_number
      puts "The balance of the account is #{account.balance}"
    elsif puts "This account does not exists!"
    end
  end
end

def whithdraw_from_account(account_number, value)
  @list_of_accounts.map do |account|
    if account.number == account_number
...
def insert_into_account(account_number, value)
  @list_of_accounts.map do |account|
    if account.number == account_number
      account.balance += value
