class Bank
	...
	def whithdraw_from_account(account,value)
		@list_of_accounts.map do |e|
			e.balance -= value
		puts "It was whitdrawed from the account #{value}. The current balance of the account is #{e.balance}."
	end

	def insert_into_account(account,value)
		@list_of_accounts.map do |e|
			e.balance += value
			puts "It was inserted on the account #{value}. The current balance of the account is #{e.balance}."
	end
